...
# ВАЖНО: ретро-данные по кварталам живут в ОТДЕЛЬНОМ файле data_history.json
# (корень репозитория, рядом с data.json). Этот скрипт НИКОГДА не должен
# создавать, читать или перезаписывать data_history.json — он заполняется
# и обновляется вручную (см. комментарий внутри самого файла). Причина
# та же, что и с CBR_PF_SYSTEM выше: всё, что пишет этот скрипт, полностью
# пересобирается с нуля при каждом запуске — ретро-архив так делать нельзя.
...
BANKS = [
  ...
  {"r":9,  "n":"Банк ДОМ.РФ",   "lo":"Д","lc":"#1565C0","tp":"state",  "pub":1,"pf":1,
   "e_id":None, "ticker":None,
   # Методология унифицирована на ГРУППУ с 22.07.2026 (см. резюме проекта).
   # Раньше стоял scope='bank': отдельной небанковской МСФО у ДОМ.РФ нет,
   # публикуется только консолидированная — сохранять "заплатку" под Банк
   # standalone (CIR 16.8% vs 29.7% группы за 1кв.2025) больше не имеет смысла:
   # ретро-данные ТОП-10 ведём единообразно по группе (см. data_history.json).
   # H20.0 всё равно берётся по банку — регулятивный норматив всегда банковский.
   # TODO (после выхода МСФО группы 1кв.2026): пересчитать base под группу.
   # Пока в base — временно цифры банка standalone 1кв.2026, чтобы live-снапшот
   # не показывал пустоту; сорс явно помечен "(временно)".
   "ir_urls":["https://domrfbank.ru/about/information/msfo/","https://domrfbank.ru/about/press/"],
   "base":{"roe":23.8,"nim":4.4,"cir":16.8,"cor":1.0,"h20":13.5,"src":"МСФО Банк ДОМ.РФ 1кв.2026 (временно; TODO пересчитать под группу когда выйдет МСФО группы 1кв.2026)","period":"1кв2026"},
   "est":{"re":0,"ne":0,"ce":0,"oe":0,"he":1},"st":"data"},
...
GUIDANCE_WORDS = re.compile(
    r'план(?:ир|ов)?\w*|прогноз\w*|ожида\w*|цел[ьи]\w*|таргет\w*|'
    r'guidance|target|forecast|к\s+20\d\d\s+году|намерен\w*|будет\s+стремиться',
    re.I
)

def _not_guidance(text, start):
    """
    Отсекает совпадения рядом с прогнозной/плановой лексикой банка
    ("CIR 30-32% к 2026 году", "цель по ROE — 22%" и т.п.) — эти цифры
    НЕ являются отчётными фактами и не должны попадать в data.json как
    текущее значение метрики. Именно так один раз подхватился таргет
    Сбербанка по CIR (30-32%) вместо отчётного факта (26.7%/1кв.2026).
    Окно ±80 символов — обычно guidance-фраза стоит вплотную к числу.
    """
    window = text[max(0, start-80):start+80]
    return not GUIDANCE_WORDS.search(window)
...
def _scope_ok(text, start, scope):
    ...
    scope='bank'  → рядом с числом не должно преобладать слово "Групп(а/ы)"
                    ...
                    а не самого банка (пример: Т-Технологии — банк и группа
                    Т-Банк/ТКС Холдинг публикуют РАЗНЫЕ NIM/CIR из-за
                    небанковских дочек — брокер, страхование и т.п.).
                    NB: Банк ДОМ.РФ с 22.07.2026 переведён на scope=None
                    (методология унифицирована на группу, см. запись в BANKS).
    ...
    for p in pats:
        for m in re.finditer(p, text, re.I | re.M):
            if not _scope_ok(text, m.start(), scope):
                continue
            if not _not_guidance(text, m.start()):
                continue
            try:
...
def extract_metrics(text, scope=None):
    ...
    scope: 'bank' — извлекать только показатели банка (не группы/холдинга).
           ...
           компания/группа отчитывается отдельно и шире (Т-Технологии,
           МТС Банк и т.п.) — иначе метрики банка и группы могут перепутаться.
           NB: Банк ДОМ.РФ с 22.07.2026 переведён на scope=None (методология
           унифицирована на группу — небанковской МСФО у него нет).
    """
    found = {}
...
def compute_kpi(banks):
    ...
    return {
        ...
        # dom_rf_pf_portfolio убран 16.07.2026 — это был захардкоженный
        # портфель ПФ БАНКА ДОМ.РФ ("1.9 трлн"), который сайт молча
        # подставлял вместо системного показателя рынка (10.3 трлн),
        # потому что pf_total_trn раньше нигде не писался в результат.
        # Теперь pf_total_trn пишется явно в main() из CBR_PF_SYSTEM.
    }
...
# добавлено 23.07.2026: domrfbank.ru (при переходе методологии ДОМ.РФ на
# группу решено не парсить сайт банка автоматически — только e-disclosure).
...
ROBOTS_BLOCKED_DOMAINS = {"akbars.ru", "www.akbars.ru", "domrfbank.ru"}
...
# ── Системные показатели ПФ/эскроу (весь рынок, не по банкам) ──────
# ВАЖНО: эти цифры НЕ парсятся автоматически — на cbr.ru нет удобного
# машиночитаемого источника для них, только PDF-презентация раз в квартал.
# Обновлять вручную при выходе нового отчёта ЦБ РФ (обычно на cbr.ru
# в разделе "Финансирование долевого строительства", PDF "О проектном
# финансировании строительства жилья в I/II/III/IV квартале ГГГГ года").
#
# ИСТОРИЯ БАГА (15-16.07.2026): раньше pf_total_trn/pf_escrow_trn/
# pf_coverage_sys существовали только как разовые ручные правки прямо
# в data.json — и стирались при каждом успешном прогоне агента, потому
# что main() пересобирает data.json с нуля из ЭТИХ переменных, а не
# читает старый файл. Заодно был мусорный хардкод-фолбэк
# kpi["dom_rf_pf_portfolio"]="1.9 трлн" (портфель ПФ САМОГО БАНКА
# ДОМ.РФ, а не рынка) — из-за отсутствия pf_total_trn в выводе JS
# на сайте молча подставлял именно его вместо системного показателя.
CBR_PF_SYSTEM = {
    "period": "1кв.2026",
    "period_date": "01.04.2026",
    "pf_total_trn": 10.3,      # портфель ПФ жилищного строительства, трлн руб.
    "pf_escrow_trn": 7.4,      # средства на счетах эскроу, трлн руб.
    "pf_coverage_sys": 68.0,   # официальный % покрытия задолженности застройщиков (ЦБ РФ)
    "pf_coverage_sys_qoq_pp": -1.0,
    "pf_coverage_sys_yoy_pp": -4.3,
    "pf_coverage_sys_note": (
        "Первоисточник: ЦБ РФ, «О проектном финансировании строительства "
        "жилья в I квартале 2026 года», слайд 5. Готовое значение ЦБ — "
        "НЕ пересчитывать как эскроу/портфель_ПФ (методология ЦБ шире: "
        "полная задолженность застройщиков, а не только наш портфель ПФ)."
    ),
    "pf_source": "ЦБ РФ 1кв.2026 (01.04.2026)",
    "pf_source_url": "https://cbr.ru/Collection/Collection/File/62038/pf_1Q2026.pdf",
    "pf_cbr_page": "https://cbr.ru/statistics/bank_sector/equity_const_financing/",
}

# Статичные данные из последнего отчёта ЦБ РФ по долям ПФ банков
...
# ── CLOUDFLARE WORKER: АВТОДЕПЛОЙ И ПРОКСИ ───────────────────────────
# Агент сам деплоит Worker при первом запуске если есть CF_API_TOKEN.
# Нужен только 1 секрет в GitHub: CF_API_TOKEN
# Получить: dash.cloudflare.com → My Profile → API Tokens → Create Token
# Шаблон: "Edit Cloudflare Workers" → Create Token

import os, json as _json

CF_API_TOKEN  = os.environ.get('CF_API_TOKEN', '')   # GitHub Secret
CF_ACCOUNT_ID = os.environ.get('CF_ACCOUNT_ID', '')  # GitHub Secret
WORKER_NAME   = 'cbr-proxy'

# Код Worker (JavaScript) — проксирует cbr.ru через российские ноды CF
WORKER_JS = """
export default {
  async fetch(request) {
    ...
    // Разрешаем только cbr.ru
    const targetHost = new URL(target).hostname;
    if (!targetHost.endsWith('cbr.ru')) {
...
def get_worker_url() -> str:
    """
    Возвращает URL Worker. Если Worker не задеплоен — деплоит автоматически.
    Требует: CF_API_TOKEN и CF_ACCOUNT_ID в переменных окружения.
    """
    if not CF_API_TOKEN or not CF_ACCOUNT_ID:
        return ''

    # Проверяем существует ли уже Worker
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            ...
            if data.get('success'):
                url = f'https://{WORKER_NAME}.{CF_ACCOUNT_ID[:8]}.workers.dev'
                print(f"    ✓ Worker уже существует: {url}")
                return url
    except urllib.error.HTTPError as e:
        if e.code == 404:
            print(f"    → Worker не найден, деплоим...")
        else:
            print(f"    WARN check worker: HTTP {e.code}")
    except Exception as e:
        print(f"    WARN check worker: {e}")

    # Деплоим Worker через Cloudflare API
    return deploy_worker()

def deploy_worker() -> str:
    """Деплоит Worker через Cloudflare API. Не требует npm или wrangler."""
    if not CF_API_TOKEN or not CF_ACCOUNT_ID:
        return ""

    print(f"    → Деплоим Worker через Cloudflare API...")

    # Cloudflare Workers API: PUT script
    # Используем application/javascript для простого (non-module) Worker
    worker_code = """
...
async function handleRequest(request) {
  ...
  try {
    const targetHost = new URL(target).hostname;
    if (!targetHost.endsWith("cbr.ru")) return new Response("Forbidden", {status: 403});
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as r:
            ...
            if result.get("success"):
                subdomain = get_workers_subdomain()
                url = f"https://{WORKER_NAME}.{subdomain}.workers.dev"
                print(f"    ✓ Worker задеплоен: {url}")
                return url
            else:
                print(f"    ✗ Ошибка: {result.get(chr(101)+chr(114)+chr(114)+chr(111)+chr(114)+chr(115), [])}")
                return ""
    except Exception as e:
        print(f"    ✗ Деплой не удался: {e}")
        return ""

def get_workers_subdomain() -> str:
    """Получает subdomain аккаунта для workers.dev URL."""
    try:
        req = urllib.request.Request(
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            data = _json.loads(r.read())
            return data.get('result', {}).get('subdomain', CF_ACCOUNT_ID[:8])
    except:
        return CF_ACCOUNT_ID[:8]

def fetch_via_proxy(url: str) -> bytes:
    """Загружает URL через Cloudflare Worker или напрямую."""
    worker_url = get_worker_url()
    if not worker_url:
        return fetch_direct(url)

    proxy_url = f"{worker_url}/?url={urllib.parse.quote(url, safe='')}"
    try:
...
def main():
    ...
    for bank_def in BANKS:
        ...
        #
        # ДОПОЛНИТЕЛЬНО (16.07.2026): сверяем с "якорем" base = bank_def["base"]
        # (курируемые вручную значения). Если ROE или CIR отличаются от base
        # больше чем на 25% относительно — это подозрительно (могла подхватиться
        # цифра ГРУППЫ вместо банка, ПЛАН/прогноз вместо факта, чужой период
        # и т.п. — весь этот класс багов мы сегодня уже дважды ловили руками).
        # Такой снапшот не используем молча — печатаем предупреждение и берём
        # следующий по полноте кандидат, прошедший проверку; если ни один не
        # прошёл — sl_data остаётся None, и ниже сработает fallback на base.
        def _diverges_from_base(metrics, base_dict, threshold=0.25):
            for k in ("roe", "cir"):
                bv, mv = base_dict.get(k), metrics.get(k)
                if bv and mv and bv != 0:
                    if abs(mv - bv) / abs(bv) > threshold:
                        return True, k, bv, mv
            return False, None, None, None

        candidates = sorted(
            sl_snapshots.get(bank_def["n"], []),
            key=lambda s: len(s["metrics"]), reverse=True
        )
        sl_data = None
        for cand in candidates:
            bad, mkey, bv, mv = _diverges_from_base(cand["metrics"], base)
            if bad:
                print(f"    ⚠ {bank_def['n']}: снапшот '{cand['src']}' отклонён — "
                      f"{mkey} {mv} расходится с base {bv} более чем на 25% "
                      f"(похоже на группу/план/чужой период, а не факт по банку)")
                continue
            sl_data = dict(cand["metrics"])
            sl_data["src"] = cand["src"]
            break
    ...
    result = {
        ...
        # Системные показатели ПФ/эскроу — из CBR_PF_SYSTEM (см. выше),
        # обновляется вручную при выходе нового квартального отчёта ЦБ РФ.
        **CBR_PF_SYSTEM,
        "asset_rank_source": "1000bankov.ru / ЦБ РФ форма 101, апрель 2026",
        "asset_rank_url": "https://1000bankov.ru/rating/",
        "concept": "ТОП-20 банков по портфелю ПФ, ранжированных по активам",
    }
